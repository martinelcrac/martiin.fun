...
if getgenv().Config.Norender then
...
if not getgenv().Config.Norender then
...
--fireproximityprompt(game:GetService("Workspace")["Tomar pastel"].Part["Prompt Attachment"].ProximityPrompt, 1)
	...
	if var == getgenv().Config.Tools_max then
		break
	end
end

if getgenv().Config.Nosimradius then
	wait(1)
	local fenv = getfenv()
...
if not getgenv().Config.Nosimradius then
